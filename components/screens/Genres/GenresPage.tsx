...
interface IGenresPage {
  route: RouteProp<TypeRootStackParamList, "GenresPage">;
}

const GenresPage: FC<IGenresPage> = ({ route }) => {
  ...
  const GetAllTracks = async () => {
    try {
      ...
      const { data } = await axios.get<IOperationResult<ITrack[]>>(
        `${API_URL}/Statistics/get-popular-tracks-by-genre?genreId=${route?.params?.genre.id}`
      );
      if (data.success) {
